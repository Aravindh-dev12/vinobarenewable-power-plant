...
function keep(newValue,oldValue){return newValue===null||newValue===undefined?oldValue:newValue;}
...
function updateInverter(message){
    ...
    const power=activePower(values),reactive=exactOr(values,'a.c. reactive power',[k=>/reactive.*power/.test(k)]),pf=exactOr(values,'Power Factor',[k=>/power.*factor|cosphi/.test(k)]);
    const vacAB=exactOr(values,'a.c. voltage AB',[k=>/voltage.*ab|ab.*voltage/.test(k)]),vacBC=exactOr(values,'a.c. voltage BC',[k=>/voltage.*bc|bc.*voltage/.test(k)]),vacCA=exactOr(values,'a.c. voltage CA',[k=>/voltage.*ca|ca.*voltage/.test(k)]);
    const freq=exactOr(values,'a.c. frequency',[k=>/frequency|freq/.test(k)]),iA=exactOr(values,'A phase current',[k=>/a.*phase.*current/.test(k)]),iB=exactOr(values,'B phase current',[k=>/b.*phase.*current/.test(k)]),iC=exactOr(values,'C phase current',[k=>/c.*phase.*current/.test(k)]);
    const eff=exactOr(values,'inverter efficiency',[k=>/efficiency/.test(k)]),amb=exactOr(values,'internal ambient temperature',[k=>/ambient.*temp|internal.*temp/.test(k)]),daily=dailyGeneration(values);
    const totalGen=exactOr(values,'total generation',[k=>/total.*generation/.test(k)]),dailyCO2=exactOr(values,'daily CO2 reduction',[k=>/daily.*co2/.test(k)]),totalCO2=exactOr(values,'total CO2 reduction',[k=>/total.*co2/.test(k)]);
    invData[key]={
        ...old,name:key,deviceName:String(message.device||key),received:true,
        power:keep(power,old.power),reactive:keep(reactive,old.reactive),pf:keep(pf,old.pf),
        vac_ab:keep(vacAB,old.vac_ab),vac_bc:keep(vacBC,old.vac_bc),vac_ca:keep(vacCA,old.vac_ca),
        freq:keep(freq,old.freq),i_a:keep(iA,old.i_a),i_b:keep(iB,old.i_b),i_c:keep(iC,old.i_c),
        eff:keep(eff,old.eff),amb:keep(amb,old.amb),dailyGen:keep(daily,old.dailyGen),totalGen:keep(totalGen,old.totalGen),dailyCO2:keep(dailyCO2,old.dailyCO2),totalCO2:keep(totalCO2,old.totalCO2),
...
function renderAll(){
    const names=Object.keys(invData).sort((a,b)=>invNo(a)-invNo(b)||a.localeCompare(b)),received=names.filter(k=>invData[k].received),withPower=names.filter(k=>invData[k].power!==null&&invData[k].power!==undefined),withDaily=names.filter(k=>invData[k].dailyGen!==null&&invData[k].dailyGen!==undefined);
    document.getElementById('inv_total_count').textContent=EXPECTED_COUNT;
    document.getElementById('inv_total').innerHTML=(withPower.length?withPower.reduce((s,k)=>s+num(invData[k].power),0).toFixed(2):'--')+' <span class="text-sm text-blue-600">kW</span>';
    document.getElementById('inv_total_gen').innerHTML=(withDaily.length?withDaily.reduce((s,k)=>s+num(invData[k].dailyGen),0).toFixed(2):'--')+' <span class="text-sm text-purple-600">kWh</span>';
    const effs=received.map(k=>invData[k].eff).filter(v=>v!==null&&v!==undefined);document.getElementById('inv_avg_eff').innerHTML=(effs.length?(effs.reduce((a,b)=>a+Number(b),0)/effs.length).toFixed(1):'--')+' <span class="text-sm text-sky-600">%</span>';
    const container=document.getElementById('inv_detail_container');
    container.innerHTML=names.map(name=>{
        const v=invData[name],strings=v.strings||[],active=strings.filter(x=>x.active).length,expected=expectedStrings(name),hasPower=v.power!==null&&v.power!==undefined,status=!hasPower?'WAITING':num(v.power)>0.01?'ONLINE':'OFFLINE',statusClass=!hasPower?'bg-slate-100 text-slate-500':num(v.power)>0.01?'bg-emerald-100 text-emerald-700':'bg-red-100 text-red-700',stringText=v.stringsSeen?`${active}/${expected} strings`:`--/${expected} strings`;
