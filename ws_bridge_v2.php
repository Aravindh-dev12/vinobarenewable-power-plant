...
function inverterPower(array $v): float {
    foreach($v as $k=>$x){$l=strtolower((string)$k);if(preg_match('/active.*power|ac.*power|power.*ac|a\.c\..*power/',$l)&&!preg_match('/reactive|apparent|3.phase/',$l))return (float)$x;}
    return 0;
}
function inverterStrings(array $v): array {
    ...
    foreach($v as $k=>$x){
        ...
        if(!preg_match('/\d/',$k)||!preg_match('/curr|current|amp/i',$k))continue;
        if(preg_match('/phase|3.phase|reactive|apparent|freq|temp|inverter.*curr|total.*curr|grid.*curr|load.*curr|mppt.*curr|dc.*curr/',$l))continue;
        $n=preg_match('/(\d+)/',$k,$m)?(int)$m[1]:$t+1;
        $cur=(float)$x;
        $strings[]=['n'=>$n,'current'=>$cur,'active'=>$cur>.5?1:0];
        $t++;if($cur>.5)$a++;
    }
    return [$a,$t,$strings];
}
function insertInverter(mysqli $conn,string $unit,array $d): void {
    $v=$d['values']??[];if(!$v)return;
    $dev=(string)($d['device']??'Inverter');$pwr=inverterPower($v);$daily=valueByRegex($v,'/daily.*generation|daily.*gen/');$total=valueByRegex($v,'/total.*generation/');$temp=valueByRegex($v,'/internal.*temp|internal.*ambient/');
    [$active,$count,$strings]=inverterStrings($v);$status=$pwr>.01?'online':'offline';
    $st=$conn->prepare('INSERT INTO inverter_readings (plant_id,device_name,ac_active_power,internal_temp,daily_generation,total_generation,active_strings,total_strings,status) VALUES (?,?,?,?,?,?,?,?,?)');
    if($st){$st->bind_param('ssddddiis',$unit,$dev,$pwr,$temp,$daily,$total,$active,$count,$status);$st->execute();$st->close();}
    foreach($strings as $s){
        $volt=0.0;$ss=$conn->prepare('INSERT INTO inverter_strings (plant_id,inverter_name,string_number,current,voltage,is_active) VALUES (?,?,?,?,?,?)');
        if($ss){$n=$s['n'];$cur=$s['current'];$act=$s['active'];$ss->bind_param('ssiddd',$unit,$dev,$n,$cur,$volt,$act);$ss->execute();$ss->close();}
    }
}
function insertVcb(mysqli $conn,string $unit,array $d): void {
    $v=$d['values']??[];if(!$v)return;
    $power=(float)($v['3 Phase Active Power']??0);$export=(float)($v['Active Total Export']??0);$tag=$d['virtualTags']['vcb-today']??null;$today=(float)(is_array($tag)?($tag['value']??0):($tag??0));
    $st=$conn->prepare('INSERT INTO vcb_readings (plant_id,active_power_total,active_total_export,today_energy) VALUES (?,?,?,?)');
    if($st){$st->bind_param('sddd',$unit,$power,$export,$today);$st->execute();$st->close();}
}
function insertTransformer(mysqli $conn,string $unit,array $d): void {
    $v=$d['values']??[];if(!$v)return;
    $dev=(string)($d['device']??'Transformer');$oil=array_key_exists('oil-temp',$v)?(float)$v['oil-temp']:null;$wind=array_key_exists('winding-temp',$v)?(float)$v['winding-temp']:null;$status=(($oil!==null&&$oil>80)||($wind!==null&&$wind>100))?'warning':'normal';
    $st=$conn->prepare('INSERT INTO transformer_readings (plant_id,device_name,oil_temp,winding_temp,status) VALUES (?,?,?,?,?)');
    if($st){$st->bind_param('ssdds',$unit,$dev,$oil,$wind,$status);$st->execute();$st->close();}
}
function insertWeather(mysqli $conn,string $unit,array $d): void {
    $v=$d['values']??[];$rad=(float)($v['raw data']??0);$temp=(float)($v['pannel temperature']??0);$wind=(float)($v['windspeed']??0);
    $st=$conn->prepare('INSERT INTO weather_readings (plant_id,radiation,panel_temp,wind_speed) VALUES (?,?,?,?)');
    if($st){$st->bind_param('sddd',$unit,$rad,$temp,$wind);$st->execute();$st->close();}
}
$lastTelemetry=[];
function telemetry(mysqli $conn,string $unit,string $type,float $value): void {
    global $lastTelemetry;
    $key="$unit|$type";$now=time();if(isset($lastTelemetry[$key])&&$now-$lastTelemetry[$key]<3600)return;$lastTelemetry[$key]=$now;
...
function processPlantMessage(mysqli $conn,string $expectedPlant,array $allowed,array $d): void {
    ...
    $unit=(string)$d['unit_id'];
    if($unit!==$expectedPlant||!isset($allowed[$unit]))return;

    $task=strtolower((string)($d['task']??''));
    $dev=strtolower((string)($d['device']??''));
    $v=$d['values']??[];
    $vcbKeys=isset($v['3 Phase Active Power'])&&(isset($v['R Phase-N Voltage'])||isset($v['Active Total Export']));

    if($task==='vcb'||strpos($dev,'vcb')!==false||$vcbKeys){
        insertVcb($conn,$unit,$d);
        if(isset($v['3 Phase Active Power']))telemetry($conn,$unit,'vcb_power',(float)$v['3 Phase Active Power']);
        return;
    }
    if($task==='transformer'||strpos($dev,'transformer')!==false){insertTransformer($conn,$unit,$d);return;}
    ...
    $keys=array_keys($v);$isInv=$task==='inverter'||strpos($dev,'inverter')!==false;
    if(!$isInv){
        foreach($keys as $k){$l=strtolower($k);if(preg_match('/active.*power|ac.*power/',$l)&&!preg_match('/reactive|apparent|3.phase/',$l)){$isInv=true;break;}}
    }
    if($isInv){
        insertInverter($conn,$unit,$d);$p=inverterPower($v);if($p>0)telemetry($conn,$unit,'inverter_power',$p);
    }
}
...
while(true){
    ...
    foreach($read as $socket){
        ...
        if($frame===null||$frame['op']==='close'){
            $clients[$plant]->close();unset($clients[$plant]);$reconnectAt[$plant]=time()+5;
