...
const EXPECTED_INVERTERS = {'vinoba-1':7,'ssv':4};
...
function canonicalInverterName(name){ const n=inverterNumber(name); return n ? `Inverter ${n}` : String(name||'Inverter'); }
function expectedStrings(plantId,name){ const n=inverterNumber(name); return EXPECTED_STRINGS[plantId]?.[n] || 23; }
function invPowerReading(values){
    for(const [k,v] of Object.entries(values||{})){
        const s=k.toLowerCase();
        if(/active.*power|ac.*power|power.*ac|a\.c\..*power/.test(s) && !/reactive|apparent|3.phase/.test(s)) return {seen:true,value:num(v)};
    }
...
plants.forEach(p => {
    const st={vcbPower:0,hasVCB:false,vcbDaily:0,inverters:{},lastLive:0};
    const count=EXPECTED_INVERTERS[p.id]||0;
    for(let i=1;i<=count;i++) st.inverters[`Inverter ${i}`]={power:null,powerSeen:false,daily:null,stringStats:null,received:false};
    state[p.id]=st;
});

function inverterTotal(st){ return Object.values(st.inverters).reduce((s,x)=>s+(x.powerSeen?num(x.power):0),0); }
...
function cardHtml(p){
    return `<article class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div class="p-5 flex-1">
            ...
            <div class="mt-4 rounded-lg border border-slate-200 overflow-hidden"><div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between"><p class="text-xs font-black">INVERTER</p><span id="inv-live-${p.id}" class="text-[9px] font-bold text-slate-400 uppercase">WAITING</span></div><div id="inv-${p.id}" class="p-3 min-h-[128px] max-h-[300px] overflow-y-auto scroll"></div></div>
        </div>
...
function updatePlant(id){
    ...
    const invLive=document.getElementById('inv-live-'+id);
    if(invLive){invLive.textContent=live?'LIVE':'WAITING';invLive.className='text-[9px] font-bold uppercase '+(live?'text-emerald-600':'text-slate-400');}
...
function handleMessage(d){
    ...
    const stringStats=liveStringStats(values), powerReading=invPowerReading(values);
    const isInv=!isVCB&&(task==='inverter'||device.includes('inverter')||powerReading.seen||stringStats!==null);
    if(isInv){
        const name=canonicalInverterName(dev||'Inverter'), old=st.inverters[name]||{power:null,powerSeen:false,daily:null,stringStats:null,received:false}, daily=dailyGen(values);
        st.inverters[name]={
            power:powerReading.seen?powerReading.value:old.power,
            powerSeen:powerReading.seen||old.powerSeen,
            daily:daily===null?old.daily:daily,
            stringStats:stringStats||old.stringStats,
            received:true
        };
    }
...
function updatePortfolioStatus(){
    ...
    el.innerHTML=`<span class="dot ${all?'bg-emerald-500':some?'bg-amber-500':'bg-red-500'}"></span>${all?'LIVE':some?liveCount+'/'+plants.length+' LIVE':'WAITING'}`;
    el.className='text-xs font-bold flex items-center gap-1.5 '+(all?'text-emerald-600':some?'text-amber-600':'text-slate-500');
}
function connectWS(){
...
async function databaseFallback(id){
    ...
    try{
        ...
        names.forEach((name,i)=>{const key=canonicalInverterName(name),old=st.inverters[key]||{stringStats:null};st.inverters[key]={power:num(row['inv'+(i+1)+'_kw']),powerSeen:true,daily:num(row['inv'+(i+1)+'_kwh']),stringStats:old.stringStats||null,received:true};});
...
renderCards();
plants.forEach(p=>updatePlant(p.id));
connectWS();
