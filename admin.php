...
<main class="max-w-7xl mx-auto w-full py-6 px-4 sm:px-6">
    <div class="flex items-center justify-between gap-3 mb-5">
        <div><h2 class="text-xl font-black text-slate-800">Plant Overview</h2><p class="text-xs text-slate-500 mt-1">Live WebSocket telemetry with database fallback</p></div>
...
function cardHtml(p){return `<article class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden"><div class="p-5"><div class="flex items-start justify-between gap-4"><div class="min-w-0"><div class="flex items-center gap-2 mb-2"><span id="badge-${p.id}" class="text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500">WAITING</span></div><h2 class="text-lg font-black text-slate-800 leading-snug">${p.name}</h2><div class="flex flex-wrap gap-x-4 gap-y-1.5 mt-2 text-xs text-slate-500"><span class="font-bold text-emerald-700"><i class="fa-solid fa-bolt mr-1"></i>Service ${p.service_number}</span><span><i class="fa-solid fa-location-dot mr-1 text-slate-400"></i>${p.location||'Karur'}</span><span><i class="fa-solid fa-solar-panel mr-1 text-slate-400"></i>${p.capacity||'--'} MW</span></div></div><a href="home.php?plant=${encodeURIComponent(p.id)}&token=${encodeURIComponent(token)}" class="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shrink-0"><i class="fa-solid fa-arrow-right mr-1"></i>Open</a></div><div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5"><div class="bg-slate-50 rounded-lg p-4 border border-slate-100"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Power</p><p id="active-${p.id}" class="text-2xl font-black text-slate-800 mt-2">0.00 <span class="text-xs text-blue-600">kW</span></p><p id="activeSource-${p.id}" class="text-[10px] text-slate-500 mt-1">Waiting</p></div><div class="bg-slate-50 rounded-lg p-4 border border-slate-100"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Today Energy</p><p id="today-${p.id}" class="text-2xl font-black text-slate-800 mt-2">0.00 <span class="text-xs text-purple-600">kWh</span></p><p class="text-[10px] text-slate-500 mt-1">Current-day generation</p></div><div class="bg-slate-50 rounded-lg p-4 border border-slate-100"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Last Update</p><p id="time-${p.id}" class="text-sm font-black text-slate-800 mt-2">Waiting</p><p id="invCount-${p.id}" class="text-[10px] text-slate-500 mt-1">0 inverters detected</p></div></div><div class="mt-4 rounded-lg border border-slate-200 overflow-hidden"><div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between"><p class="text-xs font-black text-slate-700">Inverter Fleet</p><span class="text-[9px] font-bold text-slate-400 uppercase">Live summary</span></div><div id="inv-${p.id}" class="p-3 min-h-[96px] max-h-[200px] overflow-y-auto scroll"><div class="text-xs font-semibold text-slate-400 text-center py-7">Waiting for inverter telemetry...</div></div></div></div></article>`}
...
async function fallback(id){try{const t=indiaDate(),q=new URLSearchParams({tab:'inv_vcb',type:'daily',date:t.date,plant:id,token}),r=await fetch('api_reports.php?'+q,{cache:'no-store',headers:{Authorization:'Bearer '+token}}),j=await r.json();if(!j.success||!j.data?.length)return;const elapsed=j.data.filter(x=>{const m=String(x.time_label||'').match(/^(\d+):(\d+)/);return m&&Number(m[1])*60+Number(m[2])<=t.mins}),row=elapsed.at(-1)||j.data[0],st=state[id],fresh=Date.now()-st.lastLive<20000,names=j.meta?.inv_names||[];if(!fresh){names.forEach((name,i)=>{const old=st.inverters[name]||{};st.inverters[name]={...old,power:Number(row['inv'+(i+1)+'_kw']||0),daily:Number(row['inv'+(i+1)+'_kwh']||0)});st.vcbPower=Number(row.vcb_kw||0);st.hasVCB=!!j.meta?.ht_available&&st.vcbPower!==0;st.dailyEnergy=Math.max(0,...elapsed.map(x=>Number(x.inv_total_kwh||0)));st.lastUpdate='Database '+new Date().toLocaleTimeString('en-IN',{hour12:false});update(id)}}catch(_){}}
renderCards();plants.forEach(p=>{connect(p.id);setTimeout(()=>fallback(p.id),1500)});setInterval(()=>plants.forEach(p=>fallback(p.id)),30000);status();
</script>
</body>
</html>
