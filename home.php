...
async function fetchCurveHistory(){
    try{const q=new URLSearchParams({tab:'inv_vcb',type:'daily',date:curveDate(),plant:CURRENT_PLANT,interval:'5'});if(TOKEN)q.set('token',TOKEN);const response=await fetch('api_reports.php?'+q.toString(),{cache:'no-store',headers:TOKEN?{Authorization:'Bearer '+TOKEN}:{}}),json=await response.json();if(json.success&&Array.isArray(json.data)){loadReportRows(json.data);const names=json.meta?.inv_names||[];names.forEach(name=>{const key=canonicalInvName(name);if(state.inverters[key])state.inverters[key].deviceName=name;requestHistory(name);});}}catch(_){document.getElementById('chartSource').textContent='History unavailable';}
}
